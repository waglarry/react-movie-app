...
const HomeScollMovies = ({content, filter, contentTilte, subContentTilte}) => {
    const [movies, setMovies] = useState({})

    const fetchMovieData = async () => {
        try {
            const { data } = await axios.get(`https://api.themoviedb.org/3/${content}/${filter}?api_key=${API_KEY}&language=en-US&page=1`);
            setMovies(data)
        } catch (error) {
            console.log(error)
        }
