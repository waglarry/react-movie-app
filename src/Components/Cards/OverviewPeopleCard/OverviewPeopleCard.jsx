...
const OverviewPeopleCard = ({movie, selectCard}) => {
  ...
  return (
    ...
    <div className='posterBox' onClick={setOverviewContent}>
      <img className='poster' src={movie ? API_IMG+movie.profile_path  : unavailable} alt={movie.title || movie.name ? movie.title || movie.name : ""} />
      <span className='rate'>Rated - {movie.vote_average ? movie.vote_average.toFixed(1) : ""}</span>
    </div>
    <div className='details'>
      <h4 className='title'>{movie.title || movie.name ? movie.title || movie.name : ""}</h4>
...
export default OverviewPeopleCard